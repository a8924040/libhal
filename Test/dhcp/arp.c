...
//         tHaddr[0], tHaddr[1], tHaddr[2], tHaddr[3], tHaddr[4], tHaddr[5]);
    ...
    setArpMsg(&ArpMsgSend, ARPOP_REQUEST, taddr, tHaddr, myaddr, ClientHwAddr);
    ...
    while (1)
    {
        ...
        do
        {
            ...
            if (recvfrom(dhcpSocket, &ArpMsgRecv, sizeof(arpMessage), 0, (struct sockaddr *)&addr, &j) == -1)
            {
...
int sendGraArp(u_long addr)
{
    ...
    setArpMsg(&graArpMsg, ARPOP_REPLY, addr, tHaddr, addr, ClientHwAddr);
    len = sizeof(arpMessage);
...
int sendRelArp()  /* sends UNARP message, cf. RFC1868 */
{
    ...
    setArpMsg(&ReleaseArp, ARPOP_REPLY, inaddr_broadcast, tHaddr, DhcpIface.ciaddr, ClientHwAddr);
    memset(&addr, 0, sizeof(struct sockaddr));
    memcpy(addr.sa_data, IfNameExt, IfNameExt_len);
    len = sizeof(arpMessage);
