...
#define BUF_LEN 1024
    ...
    for (i = 0, ifr = conf.ifc_req; i < num; i++, ifr++)
    {
        ...
        memcpy(wrq.ifr_name, ifr->ifr_name, IFNAMESIZE);//david
        ...
        memcpy(wiredName, ifr->ifr_name, IFNAMESIZE);
...
int IsWirelessAvailable(void)
{
    ...
    for (i = 0, ifr = conf.ifc_req; i < num; i++, ifr++)
    {
        ...
        /* skip the wired If */
        memcpy(wrq.ifr_name, ifr->ifr_name, IFNAMESIZE);
        ...
        memcpy(wirelessName, ifr->ifr_name, IFNAMESIZE);
